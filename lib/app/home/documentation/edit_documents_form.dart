    ...
    if(!kIsWeb) {
      ...
      setState(() {
        filesList.add(FileData(
            result.files.first.name, fileBytes, File(result.files.single.path!))
        );
      });
    }
    setState(() {
      ...
      try {
        ...
        if (isFileRemoved = true) {
          filesList.forEach((file) async {
            await database.editFileDocumentationParticipant(widget.participantUser.userId!, file.name, file.bytes!, document);
          });
        }
